...
import { getDayOfWeekForDate, getStreak, parseSchedule, todayBR } from "../db/index";

function sundayWeekDates(weeksAgo = 0): string[] {
  ...
  async getRecap() {
    ...
    const weekStats = (dates: string[]) => {
      ...
      const perDay = dates.map((date) => {
        ...
        const scheduledHabits = habits.filter((h) => parseSchedule(h.schedule ?? "0,1,2,3,4,5,6").has(dow));
        const total = scheduledHabits.length;
        const completed = scheduledHabits.filter((h) => (h.completions ?? []).includes(date)).length;
      ...
      const habitPct =
        habitCount > 0 ? Math.round(perDay.reduce((sum, day) => sum + day.pct, 0) / perDay.length) : 0;
    ...
    if (thisWeek.habitPct > lastWeek.habitPct) {
      trend = "up";
    } else if (thisWeek.habitPct < lastWeek.habitPct) {
      trend = "down";
    }
    ...
    for (const h of habits) {
      ...
      if (!topStreak || streak > topStreak.streak) {
        topStreak = { name: h.name, icon: h.icon, streak };
      }
    }
    ...
    const habitBreakdown = habits.map((h) => {
      ...
      const count = completions.filter((date) => date >= startDate && date <= endDate).length;
